...
One Marp markdown deck per lecture (W1–W13). These are **instructor presentation materials**, not Canvas content — they are intentionally placed in a subdirectory so the course-builder's lecture loader (which globs `content/2026/lectures/*.md` non-recursively) ignores them.
...
marp content/2026/lectures/slides/w01-intro-slides.md --pdf

# or to HTML (browser-presentable)
marp content/2026/lectures/slides/w01-intro-slides.md --html

# render all 13 in one shot
for f in content/2026/lectures/slides/w*-slides.md; do
  marp "$f" --pdf
done
```
