# Forge 02 — A Skill

**Due:** Sun Sep 20, 2026 23:59 MT · **Points:** 60

## What to build

A skill in `.claude/skills/` that automates something you were about to do by hand for the third time.

## Evidence — half the grade

Fire it **both ways** — explicitly with `/name`, and by natural phrasing that matches the description. Capture both.

Commit both the artifact and its evidence to `forge/02-skill/` in your portfolio repo:

```
forge/02-skill/
  README.md      what it does, why you built it, what it must not do
  <the artifact>
  evidence/      transcripts and output
```

Submit the commit URL.

## Notes

The description is the whole game. Claude picks a skill by reading descriptions and nothing else, so name the phrases you would actually type. Write the description before the body.

## Acceptance criteria

- The artifact exists and is demonstrated working in **your own** repository.
- It solves a problem you actually had — a generic version any project could use unchanged scores half.
- `evidence/` contains a record of it running, including output.
