# The Pitch

**Due:** Sun Aug 30, 2026 23:59 MT · **Points:** 50

## What to do

One page naming the game you will grow all semester. Commit `games/pitch.md` to your repo .

Cover four things:

1. **The game, in one sentence.** Genre, and what makes it yours.
2. **The core loop.** What the player does over and over. One sentence — if you cannot state it, you do not have one yet.
3. **The feeling you are after.** Challenge, discovery, fellowship, narrative, sensation, or expression. Pick one to lead with.
4. **Two engine capabilities you will need,** and roughly when in the term.

## Why now

Every tier assignment for the rest of the course is a slice of *this* game. Picking something you care about is worth more than picking something safe — you will be looking at it in December.

Ambition is fine. The scope contract in Week 10 is where you commit to what actually ships.

## How this is graded

**Push to your repository.** The autograder runs on the push and posts its
feedback as a **GitHub issue** on that repo, scored against the rubric below.
Read the issue; that is where your feedback lives.

There is nothing to submit in Canvas. Your commit history *is* the submission,
and the commit timestamp is what the late policy measures.

## Acceptance criteria

- The game is described specifically enough that a reader could sketch the first prototype.
- The core loop is stated as an activity, not a setting.
- Two engine capabilities named, with a rough sense of sequence.
